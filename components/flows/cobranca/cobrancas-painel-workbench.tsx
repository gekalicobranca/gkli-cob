...
export function FlowCobrancaPainelWorkbench({ rows, returnQuery = '' }: { rows: Row[]; returnQuery?: string }) {
  ...
  const allSelected = novas.length > 0 && novas.every((row) => selectedIds.includes(row.id))
  ...
  const toggleAll = () => setSelectedIds(allSelected ? [] : novas.map((row) => row.id))
  const toggleGroup = (groupRows: Row[]) => {
    const ids = groupRows.filter(isNovo).map((row) => row.id)
      ...
      <Card className="p-4">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div><h2 className="text-sm font-semibold text-slate-950">Ativar cobranças selecionadas</h2><p className="mt-1 text-xs text-slate-500">Depois da ativação, as cobranças entram na esteira de cobrança ativa.</p></div>
          </div>
      ...
      <Card className="overflow-hidden p-0">
        {rows.length === 0 ? <div className="p-5"><EmptyState title="Nenhuma cobrança nova" description="Não há cobranças novas neste filtro." /></div> : <>
          <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
            <label className="inline-flex items-center gap-3 text-sm font-medium text-slate-700"><input type="checkbox" checked={allSelected} disabled={novas.length === 0} onChange={toggleAll} className="h-4 w-4 rounded border-slate-300 text-[var(--gkli-primary)]" />Selecionar todas as novas</label>
            <p className="text-sm text-slate-500">{selectedIds.length} de {novas.length} nova(s)</p>
          </div>
          <div className="divide-y divide-slate-100">{groups.map((group) => {
            const groupNovas = group.rows.filter(isNovo)
            ...
            return <details key={group.id} className="group/condominio bg-white">
              ...
              <div className="divide-y divide-slate-100 border-t border-slate-100">{group.rows.map((row) => <div key={row.id} className="grid gap-3 px-4 py-3 xl:grid-cols-[32px_minmax(280px,1fr)_150px_150px_160px_48px] xl:items-center">
                <input aria-label={`Selecionar cobrança da unidade ${row.unidade?.identificacao ?? ''}`} type="checkbox" checked={selectedIds.includes(row.id)} disabled={!isNovo(row)} onChange={() => toggleOne(row.id)} className="h-4 w-4 rounded border-slate-300 text-[var(--gkli-primary)]" />
                <div><div className="flex flex-wrap gap-2"><StatusBadge status={row.status_operacional || 'novo'} /><StatusBadge status={row.status_financeiro || 'em_aberto'} /></div><Link href={`/app/cobrancas/${row.id}`} className="mt-2 block text-sm font-semibold text-slate-950 hover:text-[var(--gkli-primary)]">{row.unidade?.bloco ? `Bloco ${row.unidade.bloco} · ` : ''}Unidade {row.unidade?.identificacao || '-'}</Link><p className="mt-1 text-xs text-slate-500">{row.unidade?.responsavel_nome || 'Responsável não informado'}</p></div>
