...
function FlowRow({ flow }: { flow: any }) {
  ...
  async function loadItens() {
    ...
    setItensError('')
    try {
      const response = await fetch(`/api/flows/acordos/${flow.id}/itens`, { cache: 'no-store' })
  ...
  return <details className="group/flow" onToggle={(event) => { if (event.currentTarget.open) void loadItens() }}>
    ...
    <div className="grid gap-4 border-t border-slate-100 bg-slate-50/70 px-5 py-4 lg:grid-cols-[1fr_auto] lg:items-center">
      <div>
        ...
        <p className="mt-1 text-xs text-slate-500">
          Criado em {formatDateTimeBR(flow.created_at)} · {itensResumo} parcela(s) no Flow · Lote {lote?.status ? lote.status : 'sem status'}
        </p>
      </div>
    ...
    <div className="border-t border-slate-100 bg-white px-4 py-3">
      ...
      {itensLoading ? (
        <div className="rounded-2xl border border-dashed border-slate-200 p-4 text-sm text-slate-500">Carregando fila do Flow...</div>
      ) : itensError ? (
        <div className="rounded-2xl border border-rose-100 bg-rose-50 p-4 text-sm text-rose-700">{itensError}</div>
      ) : itens.length ? (
