...
function FlowRow({ flow }: { flow: any }) {
  ...
  async function loadItens() {
    ...
    setItensError('')
    try {
      const response = await fetch(`/api/pre-juridico/flows/${flow.id}/itens`, { cache: 'no-store' })
    ...
    <div className="grid gap-4 border-t border-slate-100 bg-slate-50/70 px-5 py-4 lg:grid-cols-[1fr_auto] lg:items-center">
      <div>
        ...
        <p className="mt-1 text-xs text-slate-500">
          Criado em {formatDateBR(flow.created_at)} · {itensResumo} item(ns) no Flow · Lote {lote?.status ? lote.status : 'sem status'}
        </p>
      </div>
