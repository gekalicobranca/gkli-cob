...
export function IniciarProcessamento({ rows }: { rows: Row[] }) {
  ...
  const total = useMemo(() => rows.filter((row) => selected.includes(row.id)).reduce((sum, row) => sum + Number(row.valor_unidade ?? row.valor_atualizado ?? row.valor_original ?? 0), 0), [rows, selected])
  ...
  return <ListPanel><details open={rows.length > 0} className="group bg-white">
    ...
    {rows.length ? <ListRows>{rows.map((row) => <ListRow key={row.id} className="md:grid-cols-[28px_minmax(260px,1fr)_130px_150px]">
      ...
      <p className="text-sm font-semibold md:text-right">{formatCurrency(Number(row.valor_unidade ?? row.valor_atualizado ?? row.valor_original ?? 0))}</p>
