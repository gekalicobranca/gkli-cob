...
import { PRE_JURIDICO_ETAPAS, etapaPreJuridicoLabel } from '@/features/pre-juridico/etapas'
...
export function PreJuridicoCasosBoard({ casos }: { casos: any[] }) {
  const [expanded, setExpanded] = useState<string | null>(null)
  return <div className="overflow-x-auto pb-3"><div className="grid min-w-[2860px] grid-cols-9 gap-3" aria-label="Fluxo dos casos pré-jurídicos">
    {PRE_JURIDICO_ETAPAS.map((etapa) => {
      ...
      return <section key={etapa.id} className={`min-h-[360px] rounded-2xl border p-2.5 ${tones[etapa.id]}`} aria-labelledby={`etapa-${etapa.id}`}>
        ...
        <div className="space-y-2">{rows.map((caso) => {
          const condominio = relation(caso.condominio), unidade = relation(caso.unidade), acordo = relation(caso.acordo), responsavel = relation(caso.responsavel)
          ...
          return <article key={caso.id} className="rounded-xl border border-white/80 bg-white p-3 shadow-sm">
            ...
            <p className="mt-3 text-sm font-semibold text-slate-900">{formatCurrency(Number(acordo?.valor_acordado ?? 0))}</p>
