type Execucao = { status: string; origem?: string | null; created_at: string; iniciado_em?: string | null; finalizado_em?: string | null; erro_mensagem?: string | null }
...
export function resumirMonitor(execucao: Execucao, arquivo: unknown, conversao: Conversao | null, ultimoSinal: string | null, now = Date.now()) {
  ...
  if (conversao?.status === 'aguardando_validacao') {
    const automatica = ['maestro', 'maestro_agendada', 'agenda_mensal'].includes(execucao.origem ?? '')
    return { state: automatica ? 'running' : 'attention', terminal: false, detail: automatica ? 'Conversão pronta; validação e importação automáticas em andamento.' : 'Conversão pronta; aguardando validação ou importação.', online }
  }
