...
const ACTIVE_COBRANCA_STATUSES = ['novo', 'em cobrança ativa', 'em negociação']
const CLOSED_COBRANCA_STATUSES = ['acordo efetivado', 'judicializado', 'suspenso', 'quitado', 'pago']
const RISK_ACORDO_STATUSES = ['em atraso', 'rompido']
...
function normalizeStatus(status?: string | null) {
  return String(status ?? 'sem status').trim().toLowerCase()
}
...
function getCondominioName(item: { condominios?: DashboardCobranca['condominios'] }) {
...
function buildStatusDistribution(items: Array<{ status: string | null; value: number }>) {
...
function traffic(value: number, yellowAt: number, redAt: number): TrafficStatus {
...
export async function getManagementDashboard(scope: CarteiraScope) {
  ...
  const [{ data: cobrancas, error: cobrancasError }, { data: acordos, error: acordosError }] =
    await Promise.all([cobrancasQuery, acordosQuery])

  if (cobrancasError) {
    throw new Error(`Erro ao carregar dashboard/cobranças: ${cobrancasError.message}`)
  }

  if (acordosError) {
    throw new Error(`Erro ao carregar dashboard/acordos: ${acordosError.message}`)
  }
  ...
  const activeCobrancas = cobrancasList.filter((item) => {
    ...
    return !CLOSED_COBRANCA_STATUSES.includes(status)
  })
  ...
  const emNegociacao = activeCobrancas.filter(
    (item) => normalizeStatus(item.status) === 'em negociação'
  )
  ...
  const totalVencido = vencidas.reduce((sum, item) => sum + money(item.valor_atualizado), 0)
  ...
  const acordosAtivos = acordosList.filter((item) => normalizeStatus(item.status) === 'ativo')
  const acordosEmRisco = acordosList.filter((item) =>
    RISK_ACORDO_STATUSES.includes(normalizeStatus(item.status))
  )
  ...
  const parcelasAbertas = parcelas.filter((parcela) => normalizeStatus(parcela.status) !== 'paga')
  const parcelasAtrasadas = parcelasAbertas.filter((parcela) => {
    const vencimento = safeDate(parcela.vencimento)
    return Boolean(vencimento && vencimento < now)
  })

  const valorParcelasAtrasadas = parcelasAtrasadas.reduce((sum, item) => sum + money(item.valor), 0)
  const valorAcordosRisco = acordosEmRisco.reduce((sum, item) => sum + money(item.valor_acordado), 0)

  const taxaConversao =
    cobrancasList.length > 0 ? Math.round((acordosList.length / cobrancasList.length) * 100) : 0
  ...
  const riscoAcordosPercent =
    totalAcordado > 0 ? Math.round((valorAcordosRisco / totalAcordado) * 100) : 0

  const valorJudicializado = cobrancasList
    .filter((item) => normalizeStatus(item.status) === 'judicializado')
    .reduce((sum, item) => sum + money(item.valor_atualizado), 0)

  const judicializacaoPercent =
    totalEmAberto + valorJudicializado > 0
      ? Math.round((valorJudicializado / (totalEmAberto + valorJudicializado)) * 100)
      : 0

  const cobrancasSemInteracao = activeCobrancas.filter((item) => {
    const baseDate = safeDate(item.ultima_interacao_at) ?? safeDate(item.created_at) ?? safeDate(item.vencimento)
    return daysBetween(baseDate, now) >= 7
  })

  const semInteracaoPercent =
    activeCobrancas.length > 0
      ? Math.round((cobrancasSemInteracao.length / activeCobrancas.length) * 100)
      : 0
  ...
  for (const item of vencidas) {
    ...
    const bucket = agingBuckets.find((entry) => days >= entry.min && days <= entry.max)
    if (bucket) {
      bucket.count += 1
  ...
  const statusDistribution: StatusSlice[] = buildStatusDistribution(
    cobrancasList.map((item) => ({ status: item.status, value: money(item.valor_atualizado) }))
  )

  const agreementDistribution: StatusSlice[] = buildStatusDistribution(
    acordosList.map((item) => ({ status: item.status, value: money(item.valor_acordado) }))
  )

  const monthlyMap = new Map<string, { label: string; aberto: number; acordado: number }>()
  const addMonth = (dateValue: string | null, key: 'aberto' | 'acordado', value: number) => {
  ...
  const condominioMap = new Map<string, { nome: string; aberto: number; acordado: number; count: number }>()

  for (const item of activeCobrancas) {
    const nome = getCondominioName(item)
    const current = condominioMap.get(nome) ?? { nome, aberto: 0, acordado: 0, count: 0 }
  ...
  const critical90 = agingBuckets.find((item) => item.label === '+90 dias')?.value ?? 0
  ...
  const semaforos = [
    ...
    {
      label: 'Aging +90 dias',
      status: traffic(totalVencido > 0 ? Math.round((critical90 / totalVencido) * 100) : 0, 15, 30),
      value: totalVencido > 0 ? `${Math.round((critical90 / totalVencido) * 100)}%` : '0%',
  ...
  return {
    ...
    acordosEmAtraso: acordosList.filter((item) => normalizeStatus(item.status) === 'em atraso').length,
    acordosEmRisco: acordosEmRisco.length,
    judicializados: cobrancasList.filter((item) => normalizeStatus(item.status) === 'judicializado').length,
