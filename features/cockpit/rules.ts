...
export type CockpitItem = {
  ...
  ultimoEvento?: { tipo?: string; descricao?: string | null; created_at?: string | null } | null;
  href: string;
  score: number;
...
export function buildAcordoItem(acordo: any): CockpitItem {
  ...
  return {
    ...
    status,
    statusFinanceiro: acordo.status_financeiro ?? null,
    valor,
...
export function buildCobrancaItem(cobranca: any): CockpitItem {
  ...
  if ((COBRANCA_STATUS_SEM_ACAO as string[]).includes(status)) {
    score -= 40;
    acao = "Sem ação imediata";
    motivo = "Cobrança fora da fila operacional principal.";
  } else if (status === COBRANCA_STATUS.EM_NEGOCIACAO) {
    score += 45;
    acao = "Fechar acordo";
  ...
  } else if (status === COBRANCA_STATUS.NOVO) {
    score += 18;
    acao = "Iniciar contato";
    motivo = "Nova cobrança precisa de primeiro movimento.";
  } else if (status === COBRANCA_STATUS.ACORDO_FIRMADO) {
    score += 10;
