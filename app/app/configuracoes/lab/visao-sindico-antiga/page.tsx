...
function tabHref(tab: DashboardTab, data: { selectedCondominio?: CondominioOption | null; inicio: string; fim: string }) {
  ...
  return `/app/configuracoes/lab/visao-sindico-antiga?${params.toString()}`;
}
...
export default async function VisaoSindicoPage({
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        eyebrow="Configurações · Lab"
        title="Visão do síndico antiga"
        description="Dashboard executivo anterior do condomínio, preservado para consulta e comparação com a nova visão."
        actions={
