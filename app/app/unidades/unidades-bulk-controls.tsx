...
export function UnidadesBulkControls() {
  function toggleAll(checked: boolean) {
    document
      .querySelectorAll<HTMLInputElement>('input[name="unidade_ids"]')
      .forEach((input) => {
  ...
  return (
    <div className="flex flex-col gap-3 border-b border-slate-100 bg-slate-50/80 px-5 py-3 xl:flex-row xl:items-center xl:justify-between">
      <label className="inline-flex items-center gap-2 text-sm font-medium text-slate-700">
        ...
        />
        Selecionar unidades visíveis
      </label>

      <div className="grid gap-2 md:grid-cols-[220px_minmax(220px,1fr)_150px]">
        <Select name="status" defaultValue="suspensa" aria-label="Ação em lote">
          <option value="suspensa">Suspender unidade</option>
          <option value="inativa">Inativar unidade</option>
          <option value="ativa">Reativar unidade</option>
        </Select>
        <input
          name="observacao"
