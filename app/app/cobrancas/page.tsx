...
function getPriority(status: string, vencimento?: string | null) {
  ...
  if (['judicializado', 'suspenso', 'acordo efetivado'].includes(normalized)) {
  ...
  if (overdueDays >= 60 || normalized.includes('negociação')) {
    return { label: 'Alta', className: 'bg-red-50 text-red-700' }
  }

  if (overdueDays >= 30 || normalized.includes('cobrança')) {
    return { label: 'Média', className: 'bg-amber-50 text-amber-700' }
  }

  return { label: 'Nova', className: 'bg-[var(--gkli-primary-light)] text-[var(--gkli-primary)]' }
}

function sumBy(rows: any[], predicate: (row: any) => boolean) {
  return rows.filter(predicate).reduce((sum, row) => sum + Number(row.valor_atualizado ?? 0), 0)
}
...
export default async function CobrancasPage() {
  ...
  const totalEmAberto = sumBy(rows, (row: any) => !['acordo efetivado', 'judicializado', 'suspenso'].includes(row.status))
  const totalNegociacao = sumBy(rows, (row: any) => row.status === 'em negociação')
  const novas = rows.filter((row: any) => row.status === 'novo').length
  const ativas = rows.filter((row: any) => row.status === 'em cobrança ativa').length
  const emNegociacao = rows.filter((row: any) => row.status === 'em negociação').length
    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <Card className="relative overflow-hidden p-5">
          ...
          <p className="mt-3 text-2xl font-semibold tracking-tight text-slate-950">{formatCurrency(totalEmAberto)}</p>
          <p className="mt-1 text-sm text-slate-500">valor ainda em fluxo</p>
        </Card>

        {[
          ['Novas', novas, 'entrada', 'bg-[var(--gkli-primary-light)] text-[var(--gkli-primary)]'],
          ['Ativas', ativas, 'cobrança', 'bg-blue-50 text-blue-700'],
          ['Negociação', emNegociacao, formatCurrency(totalNegociacao), 'bg-amber-50 text-amber-700'],
      ...
      <Card className="overflow-hidden p-0">
        <div className="border-b border-slate-100 bg-white/80 px-5 py-4">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div>
              <h2 className="text-base font-medium text-slate-950">Fila operacional</h2>
              <p className="mt-1 text-sm text-slate-500">Clique para abrir histórico, status ou acordo.</p>
            </div>

            <div className="grid gap-2 md:grid-cols-[320px_160px_160px]">
              <div className="relative">
                <Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                <Input className="pl-9" placeholder="Buscar responsável, unidade..." />
              </div>
        ...
        {rows.length === 0 ? (
          <div className="p-5">
            <EmptyState title="Nenhuma cobrança encontrada" description="Importe ou cadastre cobranças para iniciar a operação." />
          </div>
        ) : (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 truncate text-xs text-slate-500">
                      {row.condominios?.nome ?? '-'} · Unidade {row.unidades?.identificacao ?? '-'} · Competência {row.competencia ?? '-'}
                    </p>
                  </div>

                  <div>
                    <p className="text-xs font-medium uppercase tracking-[0.16em] text-slate-400">Valor</p>
                    <p className="mt-1 text-sm font-semibold text-slate-950">{formatCurrency(Number(row.valor_atualizado))}</p>
                  </div>
                  ...
                  <div>
                    <p className="text-xs font-medium uppercase tracking-[0.16em] text-slate-400">Última interação</p>
                    <p className="mt-1 text-sm text-slate-700">{formatDateBR(row.ultima_interacao_at)}</p>
                  </div>
