    ...
    <Card className="relative overflow-hidden p-5">
      ...
      {signal ? (
        <div className="relative mt-4 flex items-center gap-2 text-[12px] text-slate-500">
          <span className={cn('h-2 w-2 rounded-full shadow-[0_0_0_4px]', trafficDotClasses[signal])} />
          Semáforo {signal}
        </div>
      ) : null}
    </Card>
    ...
    <div className={cn('rounded-2xl border px-4 py-3', trafficClasses[status])}>
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <span className={cn('h-2.5 w-2.5 rounded-full shadow-[0_0_0_4px]', trafficDotClasses[status])} />
          <p className="text-[12px] font-medium uppercase tracking-[0.12em]">{label}</p>
        </div>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-[13px] leading-5 text-slate-500">
            Score gerencial combinando risco, judicialização, aging e falta de interação.
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 flex items-center justify-center">
        <div className="relative h-40 w-40">
          <svg viewBox="0 0 120 120" className="h-40 w-40 -rotate-90">
            <circle cx="60" cy="60" r="48" fill="none" stroke="rgb(226 232 240)" strokeWidth="12" />
            <circle
              cx="60"
              cy="60"
              r="48"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-4xl font-medium tracking-[-0.05em] text-slate-950">{score}</span>
            <span className="text-[11px] font-medium uppercase tracking-[0.16em] text-slate-400">/ 100</span>
          </div>
        </div>
      </div>
    </Card>
...
function HorizontalBars({ items, valueKey = 'value' }: { items: BarItem[]; valueKey?: 'value' | 'aberto' | 'acordado' }) {
        ...
        return (
          <div key={item.label}>
            <div className="mb-1.5 flex items-center justify-between gap-3 text-[13px]">
              ...
              <span className="shrink-0 font-medium text-slate-950">{formatCurrency(value)}</span>
            </div>
            <div className="h-2 overflow-hidden rounded-full bg-slate-100">
              <div className="h-full rounded-full bg-[var(--gkli-primary)]" style={{ width: percent((value / max) * 100) }} />
            </div>
          </div>
        )
      })}
...
function StackedStatus({ items }: { items: StatusSlice[] }) {
  if (items.length === 0) {
    return <p className="text-[13px] text-slate-500">Sem dados suficientes para montar a distribuição.</p>
  }
        ...
        {items.slice(0, 5).map((item) => (
          <div key={item.label} className="flex items-center justify-between gap-4 text-[13px]">
            <div className="min-w-0">
              <p className="truncate font-medium capitalize text-slate-700">{item.label}</p>
              <p className="text-[12px] text-slate-400">{item.count} registros · {item.percentage}%</p>
            </div>
            <p className="shrink-0 font-medium text-slate-950">{formatCurrency(item.value)}</p>
          </div>
        ))}
      </div>
    </div>
...
function ColumnChart({ items }: { items: Array<{ label: string; aberto: number; acordado: number }> }) {
  const max = maxValue(items.flatMap((item) => [item.aberto, item.acordado]))

  if (items.length === 0) {
    return <p className="text-[13px] text-slate-500">Sem dados mensais suficientes.</p>
  }
      ...
      {items.map((item) => (
        <div key={item.label} className="rounded-2xl bg-slate-50 p-3">
          ...
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-white">
            <div className="h-full rounded-full bg-[var(--gkli-primary)]" style={{ width: percent((item.value / max) * 100) }} />
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="space-y-6">
      ...
      >
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {metrics.semaforos.map((item) => (
            <TrafficCard key={item.label} label={item.label} status={item.status} value={item.value} description={item.description} />
          ))}
        </div>
      </PageHeader>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <KpiCard
          ...
          icon={TrendingUp}
          signal={metrics.taxaRecuperacao >= 40 ? 'verde' : metrics.taxaRecuperacao >= 20 ? 'amarelo' : 'vermelho'}
        />
        <KpiCard
          title="Conversão"
          ...
          icon={Target}
          signal={metrics.taxaConversao >= 25 ? 'verde' : metrics.taxaConversao >= 10 ? 'amarelo' : 'vermelho'}
        />
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        ...
        <Card className="p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <div className="flex items-center gap-2">
                <LineChart size={18} className="text-[var(--gkli-primary)]" />
                <h2 className="text-[15px] font-medium tracking-[-0.015em] text-slate-950">
                  Evolução mensal
                </h2>
              </div>
              <p className="mt-1 text-[13px] leading-5 text-slate-500">
                Comparativo visual entre valor em aberto e valor acordado por mês.
              </p>
            </div>
            <div className="flex items-center gap-4 text-[12px] text-slate-500">
              <span className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-slate-400" />Aberto</span>
              <span className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[var(--gkli-primary)]" />Acordado</span>
            </div>
          </div>
      ...
      <section className="grid gap-5 xl:grid-cols-3">
        <Card className="p-5">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[15px] font-medium tracking-[-0.015em] text-slate-950">Aging da inadimplência</h2>
          </div>
          <p className="mt-1 text-[13px] leading-5 text-slate-500">
            Quanto mais deslocado para +90 dias, maior a pressão sobre conversão e judicialização.
          </p>
            ...
            {metrics.agingBuckets.map((bucket) => (
              <div key={bucket.label}>
                <div className="mb-1.5 flex items-center justify-between gap-3 text-[13px]">
                  ...
                  <span className="font-medium text-slate-950">{formatCurrency(bucket.value)}</span>
                </div>
                <div className="h-2 overflow-hidden rounded-full bg-slate-100">
                  <div
                    className="h-full rounded-full bg-[var(--gkli-primary)]"
                    style={{ width: percent((bucket.value / Math.max(1, metrics.totalVencido)) * 100) }}
                  />
                </div>
                <p className="mt-1 text-[11px] text-slate-400">{bucket.count} cobranças</p>
              </div>
            ))}
          </div>
        </Card>
        ...
        <Card className="p-5">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[15px] font-medium tracking-[-0.015em] text-slate-950">Status das cobranças</h2>
          </div>
          <p className="mt-1 text-[13px] leading-5 text-slate-500">
        ...
        <Card className="p-5">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[15px] font-medium tracking-[-0.015em] text-slate-950">Funil de recuperação</h2>
          </div>
          <p className="mt-1 text-[13px] leading-5 text-slate-500">
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(360px,0.75fr)]">
        <Card className="p-5">
          <div className="flex items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                <CircleDollarSign size={18} className="text-[var(--gkli-primary)]" />
                <h2 className="text-[15px] font-medium tracking-[-0.015em] text-slate-950">
                  Condomínios com maior impacto financeiro
                </h2>
              </div>
              <p className="mt-1 text-[13px] leading-5 text-slate-500">
                Ranking combinado de valor em aberto e acordado para orientar gestão de carteira.
              </p>
            </div>
          </div>
        ...
        <Card className="p-5">
          <div className="flex items-center gap-2">
            <AlertTriangle size={18} className="text-[var(--gkli-primary)]" />
            <h2 className="text-[15px] font-medium tracking-[-0.015em] text-slate-950">Acordos sob pressão</h2>
          </div>
          <p className="mt-1 text-[13px] leading-5 text-slate-500">
            Controle gerencial dos acordos que podem virar perda de recuperação.
          </p>

          <div className="mt-5 grid gap-3">
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-slate-400">Valor em risco</p>
              <p className="mt-2 text-2xl font-medium tracking-[-0.035em] text-slate-950">{formatCurrency(metrics.valorAcordosRisco)}</p>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-slate-400">Parcelas atrasadas</p>
              <p className="mt-2 text-2xl font-medium tracking-[-0.035em] text-slate-950">{formatCurrency(metrics.valorParcelasAtrasadas)}</p>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-slate-400">Acordos em atraso</p>
              <p className="mt-2 text-2xl font-medium tracking-[-0.035em] text-slate-950">{metrics.acordosEmAtraso}</p>
            </div>
          </div>
        </Card>
      </section>
