...
function actionHref(item: { id: string; href: string; acao: string; tipo?: string }) {
...
export default async function InboxOperacionalPage({ searchParams }: InboxPageProps) {
        ...
        <div className="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
              A fila única organiza cobranças, acordos e pendências por urgência. A funcionalidade completa continua existindo; aqui aparece só o que precisa virar ação agora.
            </p>
          </div>
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <Card className="p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">Fila do dia</p>
              <p className="mt-2 text-2xl font-semibold text-slate-950">{cockpit.metrics.totalPrioridades}</p>
            </div>
            <span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#edf8fb] text-[#04799a]"><Target size={19} /></span>
          </div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">Críticos</p>
              <p className="mt-2 text-2xl font-semibold text-slate-950">{cockpit.metrics.criticos}</p>
            </div>
            <span className="grid h-11 w-11 place-items-center rounded-2xl bg-red-50 text-red-700"><AlertTriangle size={19} /></span>
          </div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">Carteira acionável</p>
              <p className="mt-2 text-2xl font-semibold text-slate-950">{formatCurrency(cockpit.metrics.carteiraAcionavel)}</p>
            </div>
            <span className="grid h-11 w-11 place-items-center rounded-2xl bg-blue-50 text-blue-700"><WalletCards size={19} /></span>
          </div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">Conversão</p>
              <p className="mt-2 text-2xl font-semibold text-slate-950">{formatCurrency(cockpit.metrics.potencialConversao)}</p>
            </div>
            <span className="grid h-11 w-11 place-items-center rounded-2xl bg-amber-50 text-amber-700"><Handshake size={19} /></span>
          </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)_360px]">
        <Card className="h-fit p-3">
          <div className="px-2 py-2">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Filas inteligentes</p>
            <p className="mt-1 text-sm text-slate-500">Escolha o contexto. Sem tabela pesada.</p>
          </div>
          ...
          <div className="mt-2 space-y-2">
            {filas.map((item) => {
              ...
              return (
                ...
                >
                  <span className={['grid h-10 w-10 place-items-center rounded-2xl', active ? 'bg-white text-[#04799a]' : 'bg-slate-100 text-slate-500'].join(' ')}>
                    <Icon size={17} />
                  </span>
        ...
        <Card className="overflow-hidden p-0">
          <div className="border-b border-slate-100 bg-white px-5 py-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-center gap-3">
                <span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#edf8fb] text-[#04799a]"><SelectedIcon size={19} /></span>
                <div>
                  <h2 className="text-base font-semibold text-slate-950">{selected.label}</h2>
          ...
          {selected.items.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <h3 className="mt-3 text-base font-semibold text-slate-950">Fila limpa</h3>
              <p className="mt-1 text-sm text-slate-500">Nenhum item encontrado para este contexto.</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${priorityClasses(item.prioridade)}`}>
                        {item.prioridade}
                      </span>
                    ...
                    <h3 className="mt-2 truncate text-sm font-semibold text-slate-950">{item.titulo}</h3>
                    <p className="mt-1 truncate text-xs text-slate-500">{item.subtitulo}</p>
                    <p className="mt-2 text-xs font-medium text-[#04799a]">Próxima ação: {item.acao}</p>
                  </div>

                  <div>
                    <p className="text-xs text-slate-400">Valor</p>
                    <p className="mt-1 text-sm font-semibold text-slate-950">{formatCurrency(item.valor)}</p>
                  </div>

                  <div>
                    <p className="text-xs text-slate-400">Referência</p>
                    <p className="mt-1 text-sm font-medium text-slate-700">{daysText(item.dataReferencia)}</p>
                    <div className="mt-2 h-1.5 rounded-full bg-slate-100">
                      <div className={`h-1.5 rounded-full ${scoreBarClass(item.prioridade)}`} style={{ width: `${item.score}%` }} />
                    </div>
                  </div>

                  <ArrowRight className="text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-[#04799a]" size={18} />
                </Link>
              ))}
            </div>
          )}
        ...
        <Card className="h-fit p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">
            A IA aparece como contexto dentro da operação, não como módulo obrigatório. A ação principal continua nas cobranças e acordos reais.
          </p>

          <div className="mt-5 space-y-3">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Melhor próxima ação</p>
              <p className="mt-2 text-sm font-medium text-slate-900">
                {topItem ? topItem.acao : 'Acompanhar fila'}
              </p>
              <p className="mt-1 text-xs leading-5 text-slate-500">
                {topItem ? topItem.motivo : 'Nenhum item crítico encontrado agora.'}
              </p>
            </div>

            <div className="rounded-2xl border border-[#d7eef5] bg-[#f5fbfd] p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#04799a]">Orientação Lite</p>
